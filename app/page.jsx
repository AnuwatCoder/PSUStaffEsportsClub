  ...
  return (
    <header id="navbar" className="fixed inset-x-0 top-0 z-50 border-b border-cyan-400/10 bg-gray-950/55 backdrop-blur-2xl transition-all duration-300">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8" aria-label="Main navigation">
        ...
        <div className="hidden items-center gap-1 rounded-full border border-white/10 bg-white/[0.03] p-1 lg:flex">{content.navigation.map((item) => <a key={item.href} data-nav-link={item.href} className="nav-link rounded-full px-3 py-2 text-sm font-medium text-slate-300 transition hover:bg-cyan-300/10 hover:text-cyan-200" href={item.href}>{item.label}</a>)}</div>
